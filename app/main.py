...
async def infer(file: UploadFile = File(...)) -> dict:
    ...
    if file.content_type not in ['image/jpeg', 'image/png', 'image/jpg']:
        raise HTTPException(status_code=415, detail='Unsupported file type. Send JPEG or PNG.')
    ...
    for i, d in enumerate(detections):
        ...
        size_class = 'small' if area_ratio < 0.03 else ('large' if area_ratio > 0.10 else 'medium')
    ...
    for d in detection_list:
        ...
        cv2.putText(annotated, f"{d['size_class']} {d['confidence']:.2f}",
